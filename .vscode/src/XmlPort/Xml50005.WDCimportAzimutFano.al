...
xmlport 50005 "WDC import Azimut Fano"
{
    CaptionML = FRA = 'import Commande "Azimut Fano" ', ENU = 'import "Azimut Fano" Order';
    Direction = Import;
    Format = VariableText;
    TextEncoding = UTF8;
    FieldDelimiter = '"';
                ...
                begin
                    ...
                    if lsalesline.findset() then begin
                        if oldsalesline.get(oldsalesline."Document Type"::Order, lsalesline."Document No.", lsalesline."Line No.") then begin
                            ...
                            if BlockingLevel = 'N' then
                                oldsalesline.BlockingLevel := false
                            else
                                oldsalesline.BlockingLevel := true;
                            Evaluate(oldsalesline.DueDateGap, format(WorkDate() - vRequestedDeliveryDate) + 'J');
                        ...
                        if BlockingLevel = 'N' then
                            lsalesline.BlockingLevel := false
                        else
                            lsalesline.BlockingLevel := true;
                        Evaluate(lsalesline.DueDateGap, format(WorkDate() - vRequestedDeliveryDate) + 'J');
